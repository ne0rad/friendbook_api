...
const { send_message, create_chat, join_chat, get_chats, read_chat } = require("./controllers/messages");
...
io.on("connection", (socket) => {
  ...
  function execute(func, data, callback) {
    if(typeof callback !== 'function' || typeof func !== 'function') {
      return;
    }
    if (user && data) {
      ...
      callback("No user or data provided.");
    }
  }
  ...
  socket.on('read_chat', (data, callback) => execute(read_chat, data, callback));
  // ----------------------------
